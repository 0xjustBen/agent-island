import Testing
import Foundation
@testable import AgentIslandAdapters
...
@Test func mergeIntoEmpty_addsEntry() throws {
    ...
    let merged = try JSONCMerger.merge(
        original: original,
        ensureHookCommand: "/Applications/AgentIsland.app/Contents/MacOS/agentisland-bridge --source claude",
        eventName: "PreToolUse",
        matcher: "*",
        timeoutSeconds: nil
    )
    #expect(merged.contains("agentisland-bridge"))
...
@Test func mergePreservesExistingHooks() throws {
    ...
    let merged = try JSONCMerger.merge(
        original: original,
        ensureHookCommand: "/path/agentisland-bridge --source claude",
        eventName: "PreToolUse", matcher: "*", timeoutSeconds: nil
    )
    #expect(merged.contains("other-tool"))            // preserved
    #expect(merged.contains("agentisland-bridge"))      // added
}

@Test func mergeIsIdempotent() throws {
    let url = Bundle.module.url(forResource: "settings-empty", withExtension: "json")!
    let original = try String(contentsOf: url)
    let cmd = "/p/agentisland-bridge --source claude"
    ...
    let twice = try JSONCMerger.merge(original: once, ensureHookCommand: cmd,
        eventName: "PreToolUse", matcher: "*", timeoutSeconds: nil)
    let count = twice.components(separatedBy: "agentisland-bridge").count - 1
    #expect(count == 1)
}

@Test func mergeAllEvents_installs_11_entries_with_permission_timeout() throws {
    var s = "{}"
    let cmd = "/bin/sh -c '[ -x \"$HOME/.agentisland/bin/agentisland-bridge\" ] && \"$HOME/.agentisland/bin/agentisland-bridge\" --source claude; exit 0'"
    ...
    // Command literal contains "agentisland-bridge" twice; count hook entries via "type" key instead.
...
@Test func hasHook_detectsExisting() throws {
    let s = """
    { "hooks": { "PreToolUse": [
        { "matcher": "*", "hooks": [{ "type": "command", "command": "/p/agentisland-bridge --source claude" }] }
    ]}}
    """
    #expect(JSONCMerger.hasHook(in: s, command: "/p/agentisland-bridge --source claude",
