...
public enum LauncherWriter {
    ...
    public static let bundleId = "app.agentisland.macos"
    public static let appBundleName = "AgentIsland.app"
    ...
    public static func template(home: String) -> String {
        return """
        #!/bin/zsh
        # agentisland-bridge launcher (auto-generated, do not edit)
        H=/Contents/Helpers/agentisland-bridge
        B="/Applications/\(appBundleName)${H}"
        [ -x "$B" ] && exec "$B" "$@"
        for P in "/Applications/\(appBundleName)" "$HOME/Applications/\(appBundleName)"; do
          B="${P}${H}"; [ -x "$B" ] && exec "$B" "$@"
        done
        C=~/.agentisland/bin/.bridge-cache
        ...
        \(jxaCleanupBody())
        JSEOF
            rm -rf ~/.agentisland /tmp/agentisland.sock /tmp/agentisland-diagnostic.log
          fi
        else
          /bin/date +%s > "$O"
        fi
    ...
    /// JXA body that strips entries containing the substring `agentisland-bridge`
    /// from `~/.claude/settings.json` `hooks` array. Mirrors Vibe Island's JXA shape.
    private static func jxaCleanupBody() -> String {
        return #"""
        ObjC.import('Foundation')
        var h=ObjC.unwrap($.NSHomeDirectory()),m='agentisland-bridge'
